...
let answer2 = "Richtig, 04.-05. November 2024, in FSU Jena";
let second_text2 = "Falsch! ist am 04.-05. November 2024, in FSU Jena"; 
// Define the initial and secondary layers
...
let answer3 = "Richtig, KI";
let second_text3 = "Falsch! ist KI"; 
// Define the initial and secondary layers
//let enablePopupLayer3 = "myLayer3";
...
let answer4 = "Richtig, 11";
let second_text4 = "Falsch! ist 11";
// Define the initial and secondary layers
//let enablePopupLayer3 = "myLayer3";
...
let answer5 = "Richtig, Talks@eTeach";
let second_text5 = "Falsch! ist Talks@eTeach"; 
// Define the initial and secondary layers
//let enablePopupLayer3 = "myLayer3";
...
async function openPopup(doorLayer: string, enablePopupLayer: string, colliderDoorLayer: string, popupRectangle: string, popupText: string, popupAnswers: string[], correctAnswerIndex: number, answer: string, second_text: string, subscriptionIndex: number) {
    if (currentPopup) return;
    currentPopup = WA.ui.openPopup(popupRectangle, popupText, popupAnswers.map((popupAnswer, index) => ({
        label: popupAnswer,
        className: "primary",
        callback: () => {
            currentPopup?.close();
...
const subscription1 = WA.room.onEnterLayer(enablePopupLayer1).subscribe(async () => {
    await openPopup(doorLayer1, enablePopupLayer1, colliderDoorLayer1, popupRectangle, popupText, popupAnswers, 1, answer, second_text, 1);});

const subscription2 = WA.room.onEnterLayer(enablePopupLayer2).subscribe(async () => {
    await openPopup(doorLayer2, enablePopupLayer2, colliderDoorLayer2, popupRectangle2, popupText2, popupAnswers2, 0, answer2, second_text2, 2);});

const subscription3 = WA.room.onEnterLayer(enablePopupLayer3).subscribe(async () => {
    await openPopup(doorLayer3, enablePopupLayer3, colliderDoorLayer3, popupRectangle3, popupText3, popupAnswers3, 0, answer3, second_text3, 3);
});

const subscription4 = WA.room.onEnterLayer(enablePopupLayer4).subscribe(async () => {
    await openPopup(doorLayer4, enablePopupLayer4, colliderDoorLayer4, popupRectangle4, popupText4, popupAnswers4, 0, answer4, second_text4, 4);
});

const subscription5 = leaveLayerSubscription = WA.room.onEnterLayer(enablePopupLayer5).subscribe(async () => {
    await openPopup(doorLayer5, enablePopupLayer5, colliderDoorLayer5, popupRectangle5, popupText5, popupAnswers5, 1, answer5, second_text5, 5);
});
